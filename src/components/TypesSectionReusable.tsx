...
export interface TypesItemData {
  id: string;
  title: string;
  description: string;
  image: string;
...
export interface TypesSectionReusableProps {
  // Nagłówek sekcji
  sectionTitle?: string;
  ...
  // Dane typów
  typesData?: TypesItemData[];

  // Obrazy tła
  backgroundGradient?: string;
  largeFlowerImage1?: string;
  largeFlowerImage2?: string;
  decorativeFlowerImages?: string[];
...
const TypesSectionReusable: React.FC<TypesSectionReusableProps> = ({
  sectionTitle = 'Rodzaje Zasłon',
  sectionSubtitle = 'Odkryj nasze kolekcje',
  sectionDescription = 'Oferujemy zasłony dopasowane do różnych stylów wnętrz.',
  typesData = [
    {
      id: 'jedwabne',
      title: 'Jedwabne',
  ...
  backgroundGradient = 'linear-gradient(to right, rgba(249, 247, 242, 0.92), rgba(255, 255, 255, 0.85))',
  largeFlowerImage1 = '/images/background-flower/u8283414962_Detailed_blue_line_drawing_of_lily_flowers_on_whi_49aa16d2-bca9-49f0-892e-c45372365ece_3-removebg-preview.png',
  largeFlowerImage2 = '/images/background-flower/u8283414962_Detailed_blue_line_drawing_of_lily_flowers_on_whi_d4c0efab-3cd7-42e9-be57-ae5d5f0d8f2a_3-removebg-preview.png',
  decorativeFlowerImages = [
          ...
          onComplete: () => {
            if (titleRef.current?.querySelector('h2')) {
              gsap.to(titleRef.current?.querySelector('h2'), {
                textShadow: `0 0 10px rgba(${accentColor
                  .replace('#', '')
                  .match(/.{2}/g)
                  ?.map((hex) => parseInt(hex, 16))
                  .join(', ')}, 0.3), 0 0 20px rgba(${accentColor
                  .replace('#', '')
                  .match(/.{2}/g)
                  ?.map((hex) => parseInt(hex, 16))
                  .join(', ')}, 0.1)`,
  ...
  }, [accentColor, disableAnimation, titleColor]);

  // Wybór odpowiedniego układu kolumn
  ...
  // Obliczenie szerokości kolumny opisu
  const getDescriptionColumnClass = () => {
    switch (layout) {
      case 'wide':
        return 'col-lg-4 col-md-12';
      case 'compact':
        return 'col-lg-2 col-md-12';
      default:
        return 'col-lg-3 col-md-12';
    }
  };

  return (
    <section
      className={`about-section types-section py-20 relative ${className}`}
      ref={sectionRef}
      id={id}
    >
      {/* Tło z gradientem */}
      ...
      {Array(15)
        ...
        .map((_, i) => (
          ...
          >
            <Image
              src={decorativeFlowerImages[i % decorativeFlowerImages.length]}
              alt=''
              width={200}
              height={200}
              style={{
      ...
      <div className='auto-container relative z-10'>
        <div className='row clearfix'>
          {/* Kolumna z opisem */}
          <div
            className={`${getDescriptionColumnClass()} col-sm-12 content-column`}
          >
            <div className='content-box' ref={descriptionRef}>
              <div className='sec-title' ref={titleRef}>
                <p style={{ color: accentColor }}>{sectionSubtitle}</p>
                <div className='shape' style={{ background: '#dddddd' }}></div>
                <h2 style={{ color: titleColor }}>{sectionTitle}</h2>
              </div>
              <div className='text'>
                <p style={{ color: textColor }}>{sectionDescription}</p>
              </div>
            </div>
          </div>
          ...
          {typesData.map((type, index) => (
            <div
              key={type.id}
              className={`${getColumnClass()} col-sm-12 type-column`}
              ref={(el) => {
...
export default TypesSectionReusable;
